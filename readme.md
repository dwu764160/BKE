...
python3 src/simulation/train_minute_model.py                # PEC Step 2: train MPG prediction model (temporal Ridge v3 — season-N features → N+1 MPG; 16 features incl. archetype embeddings; GroupKFold CV by season; rookie lookup table from empirical draft-tier MPG; outputs minute_model_v2.pkl + minute_model_predictions_v2.parquet)
...
python3 src/simulation/season_sim.py                        # Simulation Layer 4-5: Monte Carlo season simulation + aggregation (parallel margin + PPP models, shared lineup-context + softened archetype matchup adjustments, projected wins/playoff prob/confidence bands, detailed player season/game stats generated via team stat budgets + Dirichlet usage + Beta-Binomial shooting)
python3 src/simulation/season_sim.py --single-game-home BOS --single-game-away NYK --single-game-season 2024-25 # Detailed single-game backend simulation report for one matchup
python3 src/simulation/lineup_projection.py                 # Simulation Step 2: lineup projection (starter/rotation/clutch phases with continuity + lineup-fit priors, validation using PBP first-quarter starters + substitution checks, and rotation lineups >=50 poss with <=2 starters)
...
```
Pipeline stages:
1. **project_next_season.py** — Regression-to-mean + age-adjusted carry-forward of player impact profiles (BKE/RAPM/impact_total), scenario team mapping (`end_of_season` or `preseason_snapshot`; preseason fallback uses carry-forward teams when snapshots are unavailable), draft-aware rookie generation (no CSV dependency by default), and minutes projection via the Phase 2 temporal Ridge model (pre-computed `minute_model_predictions_v2.parquet` for returning players; rookie lookup table for new entrants; roster-adaptive team normalization at `team_mpg_target=350`); manual age/impact/salary/carry-anchor multipliers removed as of Phase 2; synthetic replacement-pool rows are excluded from final forecast outputs
...
3. **season_sim.py** (forecast_mode=True) — Monte Carlo simulation against synthetic balanced schedule (fixed games per team and no truncation bias) with parallel `margin` + `ppp` model outputs; internally reuses shared lineup-projection logic for marginal starter/rotation/clutch bonuses, applies archetype-vs-archetype team interaction adjustments, and emits detailed player season/game stat outputs in addition to team win distributions; detailed player stats now flow through team stat budgets, capped Dirichlet usage competition, multinomial attempt allocation, and Beta-Binomial shooting variance before downstream box-score allocation; PPP branch uses minute-weighted team OBKE/DBKE with season-level empirical impact→net scaling, offense/defense blend control, PPP clipping, and possessions-aware sigma scaling; pace is estimated from prior-season team pace + stronger regression-to-mean (no player pace input)
4. **lineup_projection.py** (forecast_mode=True) — Projected starter/rotation/clutch lineups with continuity + lineup-fit priors and backtest-equivalent validation targets when observed historical data exists (Q1 PBP starters + top-5 clutch minutes + rotation lineups >=50 poss and <=2 starters)
...
Backtest validation: BKE carry r≈0.42-0.49, MPG carry r²=0.671 / MAE=4.06 MPG (Phase 2 temporal Ridge holdout, 2023-24→2024-25, n=426); latest end-of-season forecast-style team-level win MAE/r after lineup-context calibration: `margin` ≈ 7.75-8.83 / 0.62-0.70, `ppp` ≈ 8.44-8.74 / 0.66-0.68
